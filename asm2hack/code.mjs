export default class Code {
  ...
  dest(mne) {
    ...
    throw new Error(`Unrecognized dest mnemonic ${mne}`)
  }
  comp (mne) {
    switch (mne) {
    ...
    throw new Error(`Unrecognized comp mnemonic ${mne}`)
  }
  jump (mne) {
    switch (mne) {
      case '': return '000'
    ...
    throw new Error(`Unrecognized jump mnemonic ${mne}`)
  }
  literal (addr) {
    let num = parseInt(addr)
    return `0${num.toString(2).padStart(15, '0')}`
  }
}
