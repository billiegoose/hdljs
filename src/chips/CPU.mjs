...
CHIP CPU {
  ...
  Copy(in=instruction[0..14], out=address[0..14]);
  Copy(in=instruction[12], out=a);
  Copy(in=instruction[6..11], out=c[0..5]);
  Copy(in=instruction[3..5], out=d[0..2]);
  Copy(in=instruction[0..2], out=j[0..2]);

  // ALU computation
  Mux16(sel=a, a=A, b=inM, out=AM);
  ALU(zx=c[0], nx=c[1], zy=c[2], ny=c[3], f=c[4], no=c[5], x=D, y=AM, out=comp, zr=zr, ng=ng);

  // M register
  Copy(in=comp, out=outM[0..15]);
  And(a=i, b=d[2], out=writeM);
  Copy(in=A[0..14], out=addressM);

  // A register
  Not(in=i, out=ni);
  Or(a=ni, b=d[0], out=loadA);
  Mux16(sel=i, a=comp, b[0..14]=address, b[15]=0, out=Ain);

  // D register
  And(a=i, b=d[1], out=loadD);
  Copy(in=comp, out=Din[0..15]);
  
  // Jump computation
  Nor(a=ng, b=zr, out=ps);
  And(a=j[0], b=ng, out=j1);
  And(a=j[1], b=zr, out=j2);
  And(a=j[2], b=ps, out=j3);
  Or(a=j1, b=j2, out=jx);
  Or(a=jx, b=j3, out=jump);
  Not(in=jump, out=njump);

  // Clocked
  Register(load=loadA, in=Ain, out=A);
  Register(load=loadD, in=Din, out=D);
